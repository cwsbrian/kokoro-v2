...
triggers:
  ...
  - analisi dei log
do_not_use:
  - unit testing with test scripts
  - frontend-only testing without Docker
...
- **Docker** and **Docker Compose** — for running and streaming container logs
- **jq** — for parsing JSON log output (e.g. `jq -r '.data.path'`). Install: `apt-get install jq` (Debian/Ubuntu), `brew install jq` (macOS), or see https://stedolan.github.io/jq/download/
...
**Prerequisites**: Docker, Docker Compose, and `jq` (required for JSON log parsing). Install jq: `apt-get install jq` (Debian/Ubuntu), `brew install jq` (macOS), or see https://stedolan.github.io/jq/download/.

```bash
# Count consecutive failures on same endpoint (requires jq)
if ! command -v jq >/dev/null 2>&1; then
  echo "Error: jq is required but not installed. Install with: apt-get install jq (Linux) or brew install jq (macOS)" >&2
  exit 1
fi
